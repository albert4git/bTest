"============================== Utility functions =============================
" XXX: fnameescape vs. shellescape: for vim's consumption vs. the shell's consumption
...
if !exists('g:nnv_search_paths')

    if exists('g:nnv_directories')
        echoerr '`g:nnv_directories` has been renamed `g:nnv_search_paths`. Please update your config files.'
    else
        echoerr '`g:nnv_search_paths` is not defined.'
    endif
...
let s:ext = get(g:, 'nnv_default_extension', '.md')

" Valid options are ['up', 'down', 'right', 'left']. Default is 'right'. No colon for
" this command since it's first in the list.
let s:preview_direction = get(g:, 'nnv_preview_direction', 'right')

let s:wrap_text = get(g:, 'nnv_wrap_preview_text', 0) ? 'wrap' : ''

" Show preview unless user set it to be hidden
let s:show_preview = get(g:, 'nnv_show_preview', 1) ? '' : 'hidden'

" How wide to make preview window. 72 characters is default.
let s:preview_width = exists('g:nnv_preview_width') ? string(float2nr(str2float(g:nnv_preview_width) / 100.0 * &columns)) : ''

" Expand all directories and escape metacharacters to avoid issues later.
let s:search_paths = map(copy(g:nnv_search_paths), 'expand(v:val)')

" Separator for yanked files
let s:yank_separator = get(g:, 'nnv_yank_separator', "\n")

" The `exists()` check needs to be first in case the main directory is not
" part of `g:nnv_search_paths`.
if exists('g:nnv_main_directory')
    let s:main_dir = g:nnv_main_directory
else
    ...
    if !exists('s:main_dir')
        echomsg 'no directories found in `g:nnv_search_paths`'
        finish
    endif
endif
...
let s:create_note_key = get(g:, 'nnv_create_note_key', 'ctrl-x')
let s:yank_key = get(g:, 'nnv_yank_key', "ctrl-y")
let s:create_note_window = get(g:, 'nnv_create_note_window', 'vertical split ')

let s:keymap = get(g:, 'nnv_keymap',
            \ {'ctrl-s': 'split',
...
let s:expect_keys = join(keys(s:keymap) + get(g:, 'nnv_expect_keys', []), ',')
...
let s:use_short_pathnames = get(g:, 'nnv_use_short_pathnames', 0)
...
let s:nnv_ignore_pattern = exists('g:nnv_ignore_pattern') ? s:ignore_list_to_str(g:nnv_ignore_pattern) : ''
...
" 2iii nnv_  NNV_
command! -nargs=* -bang NNV
      \ call fzf#run(
          \ fzf#wrap({
              \ 'sink*': function(exists('*NNV_note_handler') ? 'NNV_note_handler' : '<sid>handler'),
              \ 'source': join([
                   \ 'command',
                   \ 'rg',
                   \ '--follow',
